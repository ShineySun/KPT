...
class Apollo3D(Dataset):
    ...
    def _get_anno_file_name(self):
        ...
        dataset = 'train' if 'rescore' in self.dataset else self.dataset
    ...
    def __getitem__(self, index):
        ...
        if self.data_format == 'zip':
            ...
            #print("ELSE IMREAD")
            img_name = self._get_image_path(file_name)
            #print("IMG_NAME : ", img_name)
            img = cv2.imread(
                str(self._get_image_path(file_name)),
                cv2.IMREAD_COLOR | cv2.IMREAD_IGNORE_ORIENTATION
            )
        # BGR Image
        # cv2.imshow("original_img", img)
        # cv2.imshow("resize_img", cv2.resize(img, (640, 640)))
        # cv2.imshow("output_img", cv2.resize(img, (160, 160)))
        # #print("IMG_SHAPE : ", img.shape)
        # cv2.waitKey(-1)

        cv2.imwrite("/home/sun/Desktop/DEKR_Vehicle/image_valid/" +str(img_id) + "orgin.png", img)
        cv2.imwrite("/home/sun/Desktop/DEKR_Vehicle/image_valid/" +str(img_id) + "input.png", cv2.resize(img, (640, 640)))
        cv2.imwrite("/home/sun/Desktop/DEKR_Vehicle/image_valid/" +str(img_id) + "output.png", cv2.resize(img, (160, 160)))
        #
        # # RGB Image
        ...
        if 'train' in self.dataset:
            ...
            return img, [obj for obj in target]
